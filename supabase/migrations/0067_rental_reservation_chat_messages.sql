-- Add chat messages for rental reservation updates.
...
begin
  ...
  if v_conversation_id is not null then
    v_message := 'Reservation request: ' || p_days || ' day' || case when p_days = 1 then '' else 's' end ||
      case when coalesce(p_include_driver, false) then ' with driver.' else ' without driver.' end;
    insert into public.messages (conversation_id, sender_id, body)
    values (v_conversation_id, v_renter_id, v_message);
  end if;
...
begin
  ...
  if v_conversation_id is null then
    begin
      insert into public.conversations (job_id, product_id, rental_id, accommodation_id, client_id, freelancer_id)
      values (null, null, v_rental_id, null, v_owner_id, v_renter_id)
      returning id into v_conversation_id;
    exception
      when unique_violation then
        select id
  ...
  if v_conversation_id is not null then
    v_message := case when v_decision = 'accept'
...
begin
  ...
  if v_conversation_id is not null then
    v_message := 'Reservation updated: ' || p_days || ' day' || case when p_days = 1 then '' else 's' end ||
      case when coalesce(p_include_driver, false) then ' with driver.' else ' without driver.' end;
    insert into public.messages (conversation_id, sender_id, body)
    values (v_conversation_id, v_renter_id, v_message);
  end if;
...
begin
  ...
  if v_conversation_id is not null then
    v_message := 'Reservation cancelled by renter.';
    insert into public.messages (conversation_id, sender_id, body)
    values (v_conversation_id, v_renter_id, v_message);
  end if;
...
begin
  ...
  if v_conversation_id is not null then
    v_message := 'Reservation marked completed by renter.';
    insert into public.messages (conversation_id, sender_id, body)
    values (v_conversation_id, v_renter_id, v_message);
  end if;
...
begin
  ...
  if v_conversation_id is not null then
    v_message := 'Owner marked rental as already rented.';
    insert into public.messages (conversation_id, sender_id, body)
    values (v_conversation_id, v_owner_id, v_message);
  end if;
...
begin
  ...
  if v_conversation_id is not null then
    v_message := 'Owner marked the reservation as completed.';
    insert into public.messages (conversation_id, sender_id, body)
    values (v_conversation_id, v_owner_id, v_message);
  end if;
